...
class MainDashboard_realtime(QMainWindow):
    def __init__(self):
        ...
        self.statusBar().setStyleSheet(f"color: {TEXT_SUB}; background: {CARD_BG}; border-top: 1px solid {BORDER_COLOR};")
        
        self.worker = TradingBridge()
        self.worker.data_signal.connect(self.sync_all)
        
        # --- 3. ĐÃ XÓA TỰ ĐỘNG CHẠY Ở ĐÂY ---
        self.statusBar().showMessage("⏸ Hệ thống đang chờ... Vui lòng bấm 'BẮT ĐẦU REALTIME'.")
